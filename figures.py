...
class Polygon(Figure):
    ...
    def __init__(self, canvas: tk.Canvas = None, vertexes=None):
        ...
        self.cv_lines = []
        self.cv_points = []
        # мб здесь много багов, код пока не используется
        if vertexes is not None:
            ...
            for i in range(0, len(self.vertexes) - 1):
                ...
                if self.cv is not None and len(vertexes) > 1:
                    self.cv_lines.append(self.cv.create_line(self.vertexes[i - 1].x, self.vertexes[i - 1].y,
    ...
    def add_vertex(self, p: Point):
        ...
        if self.cv is not None:
            self.cv_points.append(self.cv.create_oval(self.vertexes[-1].x - 3,
                                                      self.vertexes[-1].y - 3,
                                                      self.vertexes[-1].x + 3,
                                                      self.vertexes[-1].y + 3,
        ...
        if ln == 2:
            ...
            self.segments.append(Segment(self.vertexes[1], self.vertexes[0]))
            self.segments.append(Segment(self.vertexes[0], self.vertexes[1]))
            if self.cv is not None:
                self.cv_lines.append(self.cv.create_line(self.vertexes[1].x, self.vertexes[1].y,
            ...
            self.edges.append(Vector(self.vertexes[0] - self.vertexes[-1]))
            self.segments.pop()
            self.segments.append(Segment(self.vertexes[-1], self.vertexes[-2]))
            self.segments.append(Segment(self.vertexes[0], self.vertexes[-1]))
            if self.cv is not None:
                self.cv_lines.append(self.cv.create_line(self.vertexes[-1].x, self.vertexes[-1].y,
    ...
    def pop_vertex(self):
        self.vertexes.pop()
        self.cv.delete(self.cv_points.pop())
        self.cv.delete(self.cv_lines.pop())
        self.edges.pop()
        self.edges.pop()
        self.segments.pop()
        self.segments.pop()
